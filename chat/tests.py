...
class ChatTests(TestCase):
    ...
    def test_conversation_list_view(self):
        ...
        self.assertContains(response,
                            self.user2.username)  # Check if the participant's username is rendered in the template
    ...
    def test_start_conversation_view(self):
        self.client.login(username="user1", password="12345")
        response = self.client.get(reverse('chat:start_conversation', kwargs={'username': 'user2'}))
        self.assertRedirects(response, reverse('chat:room', kwargs={'room_name': self.conversation.id}))

    def test_delete_conversation_view(self):
        self.client.login(username="user1", password="12345")
        response = self.client.post(reverse('chat:delete_conversation', kwargs={'pk': self.conversation.id}))
    ...
    def test_get_chat_messages(self):
        self.client.login(username="user1", password="12345")
        response = self.client.get(reverse('chat:get_chat_messages', kwargs={'room_id': self.conversation.id}))
        self.assertEqual(response.status_code, 200)
